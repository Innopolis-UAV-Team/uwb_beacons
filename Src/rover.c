...
#include <stdio.h>
#include <string.h>  // transfered
#include "deca_device_api.h"  // transfered
#include "deca_regs.h"  // transfered
#include "deca_spi.h"  // transfered
#include "port.h"  // transfered
#include "stdint.h"
...
static dwt_config_t config = {  // transfered
    2,               /* Channel number. */
...
#define TX_ANT_DLY 16436  // transfered
#define RX_ANT_DLY 16436  // transfered
...
static uint8 frame_seq_nb = 0;  // transfered

/* Buffer to store received messages.
 * Its size is adjusted to longest frame that this example code is supposed to handle. */
#define RX_BUF_LEN 24  // transfered
static uint8 rx_buffer[RX_BUF_LEN];  // transfered

/* Hold copy of status register state here for reference so that it can be examined at a debug breakpoint. */
static uint32 status_reg = 0;  // transfered

/* UWB microsecond (uus) to device time unit (dtu, around 15.65 ps) conversion factor.
 * 1 uus = 512 / 499.2 �s and 1 �s = 499.2 * 128 dtu. */
#define UUS_TO_DWT_TIME 65536  // transfered
...
#define PRE_TIMEOUT 8  // transfered

/* Timestamps of frames transmission/reception.
 * As they are 40-bit wide, we need to define a 64-bit int type to handle them. */
typedef signed long long int64;
typedef unsigned long long uint64;
static uint64 poll_rx_ts;  // tx|rx changed, partially transfered
