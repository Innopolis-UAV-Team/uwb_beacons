...
int DW1000::init(void) {
    ...
    RangingData data = {0, 0, 0, 0, 0, 0, ANCHOR_ID, IDLE, 0, false};
...
void DW1000::process_msg(RangingData* data) {
    if (data->state == RangingState::PROCESSING_RESPONSE) {
        ...
        final_msg_set_ts(&final_msg[FINAL_MSG_POLL_TX_TS_IDX],
                        data->poll_tx_ts);
        final_msg_set_ts(&final_msg[FINAL_MSG_RESP_RX_TS_IDX],
                        data->resp_rx_ts);
        ...
        data->final_tx_ts = (data->resp_rx_ts + (RESP_RX_TO_FINAL_TX_DLY_UUS * UUS_TO_DWT_TIME)) >> 8;
        dwt_setdelayedtrxtime(data->final_tx_ts);
        memcpy(&final_msg[FINAL_MSG_FINAL_TX_TS_IDX], &data->final_tx_ts, FINAL_MSG_TS_LEN);
